...
class State(Enum):
    # state in which the token is home, which also is the starting position.
    HOME = 0
...
class StateSpace():
    ...
    def update_player_positions(self, players):
        ...
        for player in players:
            ...
            for piece in player.pieces:
                # if idx_player == 2:
                #     print(f'piece = {piece}')
                #     print(f'idx piece = {idx_piece}')
                #     print(f'idx player = {idx_player}')
                #     print(f'Local = {self.local_player_position[idx_player].pieces[idx_piece]}')
                if piece == 0:
                    self.global_player_position[idx_player].pieces[idx_piece] = 0
                elif piece >= 57:
                    self.global_player_position[idx_player].pieces[idx_piece] = 57
                else:
                    current = self.global_player_position[idx_player].pieces[idx_piece]
                    new = (piece + (self.quarter_game_size * idx_player)) % 52
                    if new > current:
                        self.global_player_position[idx_player].pieces[idx_piece] = new
                    else:
                        self.global_player_position[idx_player].pieces[idx_piece] = current
                # if idx_player == 2:
                #     print(f'Global = {self.global_player_position[idx_player].pieces[idx_piece]}')
    ...
    def get_global_position(self, player_idx, local_position):
        if local_position == 0:
            return 0
        elif local_position >= 57:
            return 57
        else:
            return (local_position + (self.quarter_game_size * player_idx)) % 52

    def check_if_piece_safe(self, player, piece):
        is_protected = len(np.where(
            self.local_player_position[player].pieces == self.local_position(player, piece))[0]) > 1
    ...
    def check_if_piece_is_safe_at_location(self, local_postion, global_position, is_protected):
        ...
        if local_postion >= 52:
            return True
        if local_postion != 0 and local_postion != 57 and is_protected:
            return True
        return False

    def check_if_piece_is_in_danger_at_location(self, local_postion, global_postion):
        if local_postion > 52 or local_postion == 1:
            return False
        ...
        for i in range(1, 6):
            danger_positions = np.append(
                danger_positions, np.add(self.enemyList, i))
    ...
    def get_enemy_list(self, player):
        ...
        for enemy_player_idx in range(len(self.global_player_position)):
            for enemy_piece_index in range(len(self.global_player_position[enemy_player_idx].pieces)):
                enemy_position = self.global_position(
                    enemy_player_idx, enemy_piece_index)
                enemy_local_position = self.local_position(
                    enemy_player_idx, enemy_piece_index)
                if enemy_local_position in player.pieces:
                    continue
                if enemy_position > 52 or enemy_position == 0:
                    continue
    ...
    def get_target_player_state(self, player, piece, dice):
        ...
        is_protected = len(
            np.where(self.local_player_position[player].pieces == local_pos)[0]) >= 1
    ...
    def update_move_out_action(self, player, piece, dice):
        if self.local_position(player, piece) == 0 and dice == 6:
            next_state = self.get_target_player_state(
                player, piece, dice).value
            if self.get_global_position(player, 1) in self.enemyList:
                self.update_action_table(player, Action(
                    Action.HOME_Kill.value + next_state * 9), piece, 1)
            else:
                self.update_action_table(player, Action(
                    Action.HOME_MoveOut.value + next_state * 9), piece, 1)
            return True
        return False
    ...
    def update_move_dice_action(self, player, piece, dice):
        if self.local_position(player, piece) == 0:
            return False
        if self.local_position(player, piece) + dice <= 57:
            next_state = self.get_target_player_state(
                player, piece, dice).value
            self.update_action_table(player, Action(
                Action.HOME_MoveOut.value + next_state * 9), piece, 1)
    ...
    def update_goal_action(self, player, piece, dice):
        ...
        if local_target_position == 57:
            next_state = self.get_target_player_state(
                player, piece, dice).value
            self.update_action_table(player, Action(
                Action.HOME_Goal.value + next_state * 9), piece, 1)
            return True
        return False
    ...
    def update_star_action(self, player, piece, dice):
        ...
        if (self.local_position(player, piece) + dice) in self.star_positions:
            next_state = self.get_target_player_state(
                player, piece, dice).value
            self.update_action_table(player, Action(
                Action.HOME_Star.value + next_state * 9), piece, 1)
    ...
    def update_globe_action(self, player, piece, dice):
        ...
        if (self.global_position(player, piece) + dice) in self.globe_positions_global:
            next_state = self.get_target_player_state(
                player, piece, dice).value
            self.update_action_table(player, Action(
                Action.HOME_Globe.value + next_state * 9), piece, 1)
        ...
        for i in range(len(self.local_player_position)):
            ...
            if target_position == self.local_position(player, i):
                next_state = self.get_target_player_state(
                    player, piece, dice).value
                self.update_action_table(player, Action(
                    Action.HOME_Protect.value + next_state * 9), piece, 1)
    ...
    def update_die_action(self, player, piece, dice, dieList):
        ...
        if target_position in dieList:
            next_state = self.get_target_player_state(
                player, piece, dice).value
            self.update_action_table(player, Action(
                Action.HOME_Die.value + next_state * 9), piece, 1)
    ...
    def update_goal_zone(self, player, piece, dice):
        if self.local_position(player, piece) == 0:
            return False
        local_target_position = self.local_position(player, piece) + dice
        if local_target_position > 52 and local_target_position < 57:
            next_state = self.get_target_player_state(
                player, piece, dice).value
            self.update_action_table(player, Action(
                Action.HOME_GoalZone.value + next_state * 9), piece, 1)
    ...
    def check_goal_zone(self, player, piece, dice):
        ...
        if local_target_position < 52:
            return False
        if local_position >= 52:
            self.action_table_player.set_state(State.SAFE)
        if local_target_position == 57:
            self.update_action_table(
                player, Action(Action.SAFE_Goal), piece, 1)
            return True
        self.update_action_table(player, Action(
            Action.SAFE_GoalZone), piece, 1)
        return True
